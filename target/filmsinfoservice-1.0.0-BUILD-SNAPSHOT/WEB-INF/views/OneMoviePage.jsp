...
<Style>
		*{ margin: 0; padding: 0; box-sizing: border-box;}
		...
		.like:hover{ color: #0000b3; transition: all 0.1s linear 0s; margin: 0 0 0 10px; }
		.liked:hover{ color: #000080; transition: all 0.1s linear 0s; margin: 0 0 0 10px; }
				...
				<div class="comments">
					<c:forEach items="${movie.comments}" var="comment" varStatus="i">
						<div class="${comment.commentType}">
							...
							<sec:authorize access="isAuthenticated()">
								<c:if test="${isCommentLiked[i.index] == false}"><a class="like" href="/SpringMvcExample/likeComment?rowID=${comment.rowID}&movieID=${movie.id}&commentUser=${comment.username}&movieTitle=${movie.title}">Like</a></c:if>
								<c:if test="${isCommentLiked[i.index] == true}"><a class="liked" href="/SpringMvcExample/likeComment?rowID=${comment.rowID}&movieID=${movie.id}&commentUser=${comment.username}&movieTitle=${movie.title}">Liked</a></c:if>
