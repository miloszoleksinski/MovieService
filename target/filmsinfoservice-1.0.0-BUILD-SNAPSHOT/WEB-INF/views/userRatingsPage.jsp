...
<Style>
	*{ margin: 0; padding: 0; box-sizing: border-box;}
	...
	.movieRatings{ width: 50%; margin: auto; }
	.flexRate{ display: inline-block; width: 100%; text-align: left; padding: 5px 9px 2px 9px; border-top: solid 0.5px rgb(232, 232, 232); border-left: solid 0.5px rgb(232, 232, 232); border-right: solid 0.5px rgb(232, 232, 232); }
	.ratingBorder{ width: 100%; padding: 0 8px; border: solid 0.5px rgb(232, 232, 232); margin-bottom: 30px; }
	.singleRate{ text-align: left; display: inline-block; }
	.starsInlineContainer{ display: inline-block; }
	...
	.commentTime{ font-size: 0.8em; color: grey; text-align: left; width: 100%; padding: 2px 9px 5px 2px; }
	...
	@media only screen and (max-width: 700px) 
	{
		...
		.movieRatings{ width: 90%; padding: 2%; }
	...
	<div class="fullContainer">	
		<div class="userContainer" >
				...
				<sec:authorize access="isAuthenticated()">
					<c:if test="${userModel.username != user.username}">
						<c:if test="${isUserFriend == true}">
							<a class="deleteFriendButton" id="deleteFriendMainButton" onClick="turnButton('deleteFriendMainButton')" href="<c:url value="/user/${userModel.username}/deleteFriend?redir=ratings&page=${page}"/>" >Delete friend</a><h3></h3>
						</c:if>
						<c:if test="${isUserFriend == false}">
							<a class="addFriendButton" id="addFriendMainButton" onClick="turnButton('addFriendMainButton')" href="<c:url value="/user/${userModel.username}/addFriend?redir=ratings&page=${page}"/>" >Add friend</a><h3></h3>
						</c:if>		
					</c:if>
				</sec:authorize>
			...
			<div class="userBar">
				...
				<a class="userBarOption activeBarOption" href="<c:url value="/user/${userModel.username}/ratings?page=0"/>" >Ratings (${numberOfRatings})</a>
		...
		<div class="movieRatings">
			<c:choose>
				<c:when test="${not empty notRated}">
					<p>${notRated}</p>
				</c:when>
				...
				<c:otherwise>
					<div class="titleBorder">
						<h3 class="title">Movie ratings</h3>
					</div>
					<c:forEach items="${ratings}" var="rating" varStatus="i">
						<div class="flexRate">
							<p class="singleRate"><a href="<c:url value="/movies/${rating.movieID}"/>">${moviesNames[i.index]}</a> ${rating.movieRate}</p>
							<div class="starsInlineContainer">
								<div class="ratingStars">
									<c:choose>
										<c:when test="${rating.movieRate == 1}">
										...
										<c:when test="${rating.movieRate == 2}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel">O</label>
											<label class="ratingLabel">O</label>
										...
										<c:when test="${rating.movieRate == 3}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel">O</label>
										...
										<c:when test="${rating.movieRate == 4}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
										...
										<c:when test="${rating.movieRate == 5}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
										...
										<c:when test="${rating.movieRate == 6}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
										...
										<c:when test="${rating.movieRate == 7}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
										...
										<c:when test="${rating.movieRate == 8}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
										...
										<c:when test="${rating.movieRate == 9}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
										...
										<c:when test="${rating.movieRate == 10}">
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
											<label class="ratingLabel activeRate">O</label>
							...
							<c:if test="${not empty rating.time}">
								<div class="commentTime">&bull; ${rating.time}</div>
							</c:if>	
							<c:if test="${empty rating.time}">
								<div class="commentTime"></div>
							</c:if>	
						</div>
					</c:forEach>
					<div class="ratingBorder"></div>
					
					<c:choose>
						<c:when test="${ratingPages == 1}">
 							<div class="aloneDiv">
								<a class="aloneButton" href="<c:url value="/user/${userModel.username}/ratings?page=0"/>"> 1 </a>
							</div>
  						</c:when>
 						<c:when test="${page == 0}">
 							<div class="buttonDiv0">
								<a class="middleButton middleButtonPage0" href="<c:url value="/user/${userModel.username}/ratings?page=0"/>"> 1 </a>
								<a class="spaceButton"></a>
								<a class="rightButton" href="<c:url value="/user/${userModel.username}/ratings?page=${page+1}"/>"> &gt; </a>
							</div>
  						</c:when>
  						<c:when test="${page == ratingPages-1}">
  							<div class="buttonDiv1">
        						<a class="leftButton" href="<c:url value="/user/${userModel.username}/ratings?page=${page-1}"/>"> &lt; </a>
								<a class="spaceButton"></a>
								<a class="middleButton" href="<c:url value="/user/${userModel.username}/ratings?page=0"/>"> 1 </a>
								<a class="spaceButton"></a>
								<a class="lastPageButton" href="<c:url value="/user/${userModel.username}/ratings?page=${page}"/>"> ${page+1} </a>
							</div>
  						</c:when>
  						<c:when test="${page > 0}">
  							<div class="buttonDiv2">
								...
								<a class="middleButton" href="<c:url value="/user/${userModel.username}/ratings?page=0"/>"> 1 </a>
								<a class="spaceButton"></a>
								<a class="currentPageButton" href="<c:url value="/user/${userModel.username}/ratings?page=${page}"/>"> ${page+1} </a>
								<a class="spaceButton"></a>
								<a class="rightButton" href="<c:url value="/user/${userModel.username}/ratings?page=${page+1}"/>"> &gt; </a>
							</div>
  						</c:when>
