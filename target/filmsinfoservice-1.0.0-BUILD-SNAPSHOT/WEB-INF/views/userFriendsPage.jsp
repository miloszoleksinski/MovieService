...
<Style>
	*{ margin: 0; padding: 0; box-sizing: border-box;}
	...
	.userFriends{ width: 50%; margin: auto; }
	.flexFriend{ display: flex; width: 100%; text-align: center; padding: 2%; border-top: solid 0.5px rgb(232, 232, 232); border-left: solid 0.5px rgb(232, 232, 232); border-right: solid 0.5px rgb(232, 232, 232); }
	.friendBorder{ width: 100%; padding: 0 8px; border: solid 0.5px rgb(232, 232, 232); margin-bottom: 30px; }
	.singleFriend{ text-align: center; display: inline-block; margin: 4px; }
	...
	.addFriendButton{ background-color: #009900; color: white; margin: 3px; padding: 4px 8px; border: 0.2px solid #00b300; transition: all 0.1s linear 0s; border-radius: 5px; -webkit-touch-callout:none;-webkit-user-select:none;-khtml-user-select:none;-moz-user-select:none;-ms-user-select: none;-o-user-select:none;user-select:none;}
	...
	.deleteFriendButton{ background-color: #cc0000; color: white; margin: 3px; padding: 4px 8px; border: 0.2px solid #cc0000; transition: all 0.1s linear 0s; border-radius: 5px; -webkit-touch-callout:none;-webkit-user-select:none;-khtml-user-select:none;-moz-user-select:none;-ms-user-select: none;-o-user-select:none;user-select:none;}
	...
	@media only screen and (max-width: 700px) 
	{
		...
		.userFriends{ width: 90%; padding: 2%; }
...
<title>${userModel.username} Friends</title>
</head>
<body>
	<div id="menuBorder">
	...
	<div class="fullContainer">	
		<div class="userContainer" >
				...
				<sec:authorize access="isAuthenticated()">
					<c:if test="${userModel.username != user.username}">
						<c:if test="${isUserFriend == true}">
							<a class="deleteFriendButton" id="deleteFriendMainButton" onClick="turnButton('deleteFriendMainButton')" href="<c:url value="/user/${userModel.username}/deleteFriend?redir=friends&page=${page}"/>" >Delete friend</a><h3></h3>
						</c:if>
						<c:if test="${isUserFriend == false}">
							<a class="addFriendButton" id="addFriendMainButton" onClick="turnButton('addFriendMainButton')" href="<c:url value="/user/${userModel.username}/addFriend?redir=friends&page=${page}"/>" >Add friend</a><h3></h3>
						</c:if>		
					</c:if>
				</sec:authorize>
			...
			<div class="userBar">
				...
				<a class="userBarOption activeBarOption" href="<c:url value="/user/${userModel.username}/friends?page=0"/>" >Friends (${numberOfFriends})</a>
		...
		<div class="userFriends">
			<c:choose>
				<c:when test="${not empty notAdded}">
					<p>${notAdded}</p>
				</c:when>
				...
				<c:otherwise>
					...
					<c:forEach items="${friends}" var="friend" varStatus="i">
						<div class="flexFriend">
							<p class="singleFriend"> <a href="<c:url value="/user/${friend}"/>">${friend}</a> </p>
							<sec:authorize access="isAuthenticated()">
								<c:if test="${friend != user.username}">
									<c:if test="${areUserFriendsMyFriends[i.index] == true}">
										<a class="deleteFriendButton" id="deleteFriendButtonNum${i.index}" onClick="turnButton('deleteFriendButtonNum${i.index}')" href="<c:url value="/user/${friend}/deleteFriend?redir=friends&userN=${userModel.username}&page=${page}"/>" >Delete friend</a><h3></h3>
									</c:if>
									<c:if test="${areUserFriendsMyFriends[i.index] == false}">
										<a class="addFriendButton" id="addFriendButtonNum${i.index}" onClick="turnButton('addFriendButtonNum${i.index}')" href="<c:url value="/user/${friend}/addFriend?redir=friends&userN=${userModel.username}&page=${page}"/>" >Add friend</a><h3></h3>
									</c:if>		
								</c:if>
							</sec:authorize>
						</div>
					</c:forEach>
					<div class="friendBorder"></div>
					
					<c:choose>
						<c:when test="${friendPages == 1}">
 							<div class="aloneDiv">
								<a class="aloneButton" href="<c:url value="/user/${userModel.username}/friends?page=0"/>"> 1 </a>
							</div>
  						</c:when>
 						<c:when test="${page == 0}">
 							<div class="buttonDiv0">
								<a class="middleButton middleButtonPage0" href="<c:url value="/user/${userModel.username}/friends?page=0"/>"> 1 </a>
								<a class="spaceButton"></a>
								<a class="rightButton" href="<c:url value="/user/${userModel.username}/friends?page=${page+1}"/>"> &gt; </a>
							</div>
  						</c:when>
  						<c:when test="${page == friendPages-1}">
  							<div class="buttonDiv1">
        						<a class="leftButton" href="<c:url value="/user/${userModel.username}/friends?page=${page-1}"/>"> &lt; </a>
								<a class="spaceButton"></a>
								<a class="middleButton" href="<c:url value="/user/${userModel.username}/friends?page=0"/>"> 1 </a>
								<a class="spaceButton"></a>
								<a class="lastPageButton" href="<c:url value="/user/${userModel.username}/friends?page=${page}"/>"> ${page+1} </a>
							</div>
  						</c:when>
  						<c:when test="${page > 0}">
  							<div class="buttonDiv2">
								...
								<a class="middleButton" href="<c:url value="/user/${userModel.username}/friends?page=0"/>"> 1 </a>
								<a class="spaceButton"></a>
								<a class="currentPageButton" href="<c:url value="/user/${userModel.username}/friends?page=${page}"/>"> ${page+1} </a>
								<a class="spaceButton"></a>
								<a class="rightButton" href="<c:url value="/user/${userModel.username}/friends?page=${page+1}"/>"> &gt; </a>
							</div>
  						</c:when>
