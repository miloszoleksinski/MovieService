...
<style>
	*{ margin: 0; padding: 0; box-sizing: border-box; }
	...
	#rankListLink:hover{ color: #3af; cursor: pointer; }
	...
	.contentOfTheOverlayBadges{ background-color: rgba(128,128,128,0); position: fixed; left: 15%; top: 7%; height: 86%; width: 70%; background-color: white; color: black; border-radius: 5px; padding-bottom: 20px; }
...
<body>
	<div id="badgesOverlay" class="disabled">
		<div class="contentOfTheOverlayBadges">
			...
			<div class="filterBadgesDiv">
				...
				<div class="searchBadges"> <form id="myform"><input type="text" id="badgeName" name="nameOFBadges"/></form> <i onClick="searchBadgesByTitle();changeActiveBadgesOption('searchBadgesOption');" id="searchBadgesOption1" class="material-icons searchButton">search</i> </div>
			</div>
				...
				<div class="userBadgeFlexIn">
					...
					<div class="userReputationDiv">
						<div class="reputationFlex1">
							...
							<p class="userRank"><a id="rankListLink" title="List of top 100 users" onClick="toggleAllUsers()">Rank ${userRaking} overall</a></p>
						</div>
...
<script>
	...
	// jquery ajax, get badges by its name
	function searchBadgesByTitle() 
	{
	    $.ajax({
	        ...
	        success: function(data)
	        {
	        	...
	        	if( dataLength == 0 ){ badgesID = "<p style='margin:auto; text-align: center; font-size: 1.4em; color: rgba(0,0,0,0.6);'>No results</p>" }
	...
	// prevent submit form for searching badges by its name and do it by jquery ajax (no page refreshing)
	$(document).ready(function () 
	{
	    $('#myform').on('submit', function(e) 
	    {
	        e.preventDefault();
	        changeActiveBadgesOption('searchBadgesOption');
	        searchBadgesByTitle();
	    });
	});
	
	$( "#badgesOverlay" ).on('click', function(e) {
		if (e.target === this)
		{
			toggleAllBadges();
		}
